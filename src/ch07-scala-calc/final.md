...
> **솔직한 표기 주의:** v1 35B에 대해 한때 인용되던 "~14–32초/call" 수치는 2026-05-28 pre-run 예측값(v1 ROADMAP의 사전 예측)이며 실측이 아닙니다. 부록 C가 이를 바로잡았고, 본 장도 부록 C와 동일하게 파생 측정값 약 5.3초/call을 v1 35B 기준으로 사용합니다. 이 v1.3 Scala 실행의 실측 평균은 약 5.0초/call입니다(CAPTURE-MANIFEST Timing Summary의 LLM-call gap 가중 평균 — ObservationEvent→다음 ActionEvent 간격, 즉 순수 모델 사고 시간이며 bash 실행은 제외. 태스크별 평균 2.1·6.5·2.3초를 TerminalAction 수 4·20·6으로 가중하면 ≈5.0초). 전체 active 시간은 234.8초입니다.
...
| v1.2 (6부, 2026-05-28) | Rust HTTP 서버 | Rust | 해당 없음 | unaided (1회) |
