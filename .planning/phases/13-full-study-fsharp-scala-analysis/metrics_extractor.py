#!/usr/bin/env python3
"""
metrics_extractor.py — v1.4 per-arm metrics extractor (Phase-13 patched copy).
...
PHASE-13 PATCH (user-approved 2026-06-04): the canonical-test detector was fixed.
The Phase-12 original (12-harness-rust-pilot/metrics_extractor.py, kept verbatim)
matched the FIRST ObservationEvent whose command merely CONTAINED the test
expression, so heredoc source-writes and early failed attempts produced
false-negative FAILs (Scala) and null cells (F# builds that never ran). This copy
matches only genuine run-commands and takes the LAST attempt per test, with
build-never-ran => FAIL (never null) — reproducing the JSONL-event-cited outcomes
in 13-02-RUN-NOTES.md. See _is_run_command / _extract_canonical_tests.
...
def _is_run_command(cmd, example):
    """
    True only for a genuine program-RUN invocation (not a heredoc source write).

    The Phase-12 detector matched ANY ObservationEvent whose command merely
    CONTAINED the test expression — including `cat > Calc.scala <<'EOF' ... 2+3*4 ...`
    heredoc writes and earlier failed attempts — and locked on the FIRST such match.
    That produced false-negative FAILs (e.g. Scala arm-a `2+3*4`, where the real
    PASS was a later event) and null cells when the build never ran.

    This filter restricts matching to actual run invocations so the heredoc bodies
    are excluded; the caller then takes the LAST matching run (the agent's final
    attempt for that test), which is the outcome RUN-NOTES records by event number.
    """
    if "<<" in cmd or cmd.strip().startswith("cat "):
        return False
    if example == "fsharp":
        return "dotnet run" in cmd
    if example == "scala":
        return "scala-cli run" in cmd or ("scala-cli" in cmd and ".scala" in cmd)
    return False
...
def _extract_canonical_tests(events, example):
    ...
    Canonical detection (Phase-13 fix, user-approved 2026-06-04):
      - Only genuine run-commands count (see _is_run_command) — heredoc source
        writes that happen to contain the expression are ignored.
      - A test PASSES iff some genuine run-command containing the expression exits 0
        AND emits the expected integer as a standalone output line (the program
        demonstrably computed it). Agents vary their invocation per run (separate
        runs, env-var forms, one combined command running all three tests, with
        trailing `EXIT: 0` markers), so "expected on its own line" is matched
        anywhere in the run's output, not only the last line. The FIRST such success
        is recorded; later failed re-runs do not undo a demonstrated success.
      - A test never demonstrated successfully is FAIL, not null (gate requires
        non-null). This reproduces the JSONL-event-cited PASS/FAIL/PARTIAL outcomes
        in 13-02-RUN-NOTES.md across all six F#/Scala cells.
    """
    if example in ("fsharp", "scala"):
        ...
        for idx, e in enumerate(events, start=1):  # 1-based
            ...
            if not _is_run_command(cmd, example):
                continue
            lines = [ln.strip() for ln in extract_text(obs.get("content", "")).splitlines()
                     if ln.strip()]
            line_set = set(lines)
            for expr, rec in tests.items():
                if expr not in cmd:
                    continue
                # Record last-seen actual for diagnostics (FAIL cells).
                if rec["actual"] is None:
                    rec["actual"] = lines[-1] if lines else ""
                if rec["pass"]:  # already demonstrated — don't let a later fail undo it
                    continue
                if exit_code == 0 and rec["expected"] in line_set:
                    rec["pass"] = True
                    rec["actual"] = rec["expected"]
                    rec["event_index"] = idx  # 1-based
        # never demonstrated => FAIL, never null (gate requires non-null)
