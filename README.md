...
에이전틱 AI가 무엇인지 개념부터 설명한 뒤, **OpenHands**(로컬 Qwen 3.6 35B / 122B 모델 위에서 동작)가 실제로 F# FsLex/FsYacc 계산기를 만드는 과정을 따라갑니다 — `plan → write → test → run` 루프를 돌며 스스로 빌드 오류를 고치는 모습까지요. 부록 C에서는 동일한 과제를 **두 모델로 각각 실행**한 결과를 나란히 비교합니다 — 렉서를 모델이 직접 쓸 수 있는지, 오류-수정 사이클 차이, 실측 속도.

**6부에서는 같은 35B 모델이 다른 언어로 또 하나의 예제**를 만듭니다 — 표준 라이브러리만 사용하는 최소 **Rust HTTP 서버**(`GET / → "hello\n"`). v1에서 `.fsl` 렉서를 무지원으로 쓰지 못했던 바로 그 모델이, 이번에는 `TcpListener` + `BufReader` 서버를 **무지원 첫 시도로** 작성했습니다. 두 번의 실제 빌드 실패(`format!` 구문 오류, `E0382` 소유권 오류)를 스스로 진단·수정하는 과정까지 캡처되어 있습니다. 같은 모델, 다른 언어 — Rust가 35B에게 FsLex보다 더 익숙한 영역(in-distribution)이라는 가설을 정직하게 검증합니다.
...
- **5부 — 정리와 심화**
- **6부 — 다른 워킹 예제: Rust HTTP 서버:** 같은 35B 모델이 std-only Rust HTTP 서버를 만드는 캡처된 실행(예제 소개 → 태스크 계획 → 코드 작성 → 빌드·테스트 → 완성된 서버)
- **부록 A — 재현 가이드** / **부록 B — 트러블슈팅** / **부록 C — 모델 비교(35B vs 122B)**
...
- 실제 측정된 LLM 호출 1회 평균은 **35B ≈ 5.3초**(v1 F# 계산기, 67 TerminalAction / 356초), **122B ≈ 6.3초**(150 TerminalAction / 1229초)입니다. v1.2 Rust 실행에서는 **35B ≈ 3.8초/call**(26 TerminalAction / 113초 active)로, per-call 속도는 비슷하고 실행 전체 시간 차이는 호출 횟수에서 비롯됩니다. 이 책의 초기 계획 단계에서 한 차례 추정했던 "약 14–32초/사이클"은 사전 예측치였고 실측값이 아니라는 점을 부록 C와 6부에서 함께 정정합니다.
- `.planning/`에는 이 책이 만들어진 전체 과정(계획·연구·검증, 실패했던 첫 시도 기록, 마일스톤별 감사 보고서 포함)이 그대로 공개되어 있습니다 — 정직성이라는 이 프로젝트의 핵심 가치를 따른 것입니다.
